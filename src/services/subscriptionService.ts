...
export const getMonthlySubscriptions = async (dateToFilter: Date) => {
  ...
  const currentMonthSubscriptions = allSubscriptions.filter((subscription: Subscription) =>
  ...
  const groupedByDay: Record<number, Subscription[]> =
    currentMonthSubscriptions.reduce((acc: Record<number, Subscription[]>, sub: Subscription) => {
